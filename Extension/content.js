...
function predict(data,weight){ //weight exceeded then will be malicious
    var f = 0; 
...
function isIPInURL(){ //cross site scripting sort of attack, redirected to another website
...
function isFaviconDomainUnidentical(){ //changed logo for a website
    var reg = /[a-zA-Z]\//;
    var url = window.location.href; 
...
function isAnchorFromDifferentDomain(){  //correct redirection of connection or not
...
function isScLnkFromDifferentDomain(){ //using vpn info can be redirected
...
function getIdenticalDomainCount(tag){  //repeating of various words in url
    var i;
...
var url;
function checkIfUnsafe(url) {
    const hostname = new URL(url).hostname;
  
    if (unsafeList.includes(hostname)) {
      prediction=1;
    }
  }
  
checkIfUnsafe(window.location.href);
